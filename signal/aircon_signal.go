...
func PostAirconSignal(api_secret string, aircon_appliance_id string, aircon_settings AirconSettings) int {
	...
	settingTemp := strconv.FormatFloat(aircon_settings.Temperature, 'f', 1, 64)
	// settingTempは不要な小数は落とさないといけない
	// そのため、XX.0の形式の場合、XXに変換する。たとえはば、25.0 -> 25
	// XX.5の場合はそのままの形式。たとえば、25.5 -> 25.5
	if settingTemp[len(settingTemp)-2:] == ".0" {
		settingTemp = settingTemp[:len(settingTemp)-2]
	}

	post_body := url.Values{}
	post_body.Set("temperature", settingTemp)
